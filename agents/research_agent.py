...
def _score_batch(items: list[dict], offset: int = 0, brand_context: str = "") -> list[dict]:
    # Give the model the actual evidence — title AND summary (issue #16). Scoring
    # brand relevance from the headline alone is guesswork.
    items_text = "\n".join(
        f"{offset + i + 1}. [{item.get('source', '?')}] {item['title']}\n"
        f"   Summary: {(item.get('summary') or '(no summary)')[:400]}"
...
reason: one line explaining the score"""
    ...
    # Try to parse; retry ONCE on failure. A scoring failure must NOT become a
    # passing 5.0 (issue #16) — unscored items get score=None and are excluded
    # downstream, and the failure is logged rather than silently swallowed.
    parsed = False
    for attempt in range(2):
        ...
        try:
            ...
            print(f"  [research] scoring parse failed (attempt {attempt + 1}/2): {e}")

    unscored = 0
    for item in items:
        if "score" not in item or item.get("score") is None:
            item["score"] = None       # unknown ≠ passing — excluded by the filter
            item["score_reason"] = "scoring failed" if not parsed else ""
            unscored += 1
    if unscored:
        print(f"  [research] {unscored} item(s) left UNSCORED and will be excluded")
...
def run_research(save_to_db: bool = True) -> list[dict]:
    ...
    # Default EXCLUDE (issue #16): an item with no score (scoring failed) is
