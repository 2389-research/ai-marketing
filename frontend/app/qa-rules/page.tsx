          ...
          {showForm && (
            <Card accent title="Review & save" sub="Check the wording, pick the channels, then add it.">
              <input
                ...
                disabled={saving}
                className="w-full mb-2 px-3 py-2 text-sm border border-[#cccccc] rounded outline-none focus:border-[#1800ad] disabled:opacity-50 bg-white"
              />
              <textarea
                value={ruleText}
                ...
                rows={3}
                disabled={saving}
                className="w-full mb-3 px-3 py-2 text-sm border border-[#cccccc] rounded outline-none focus:border-[#1800ad] disabled:opacity-50 resize-y bg-white"
              />
              ...
              <div className="flex gap-1.5 flex-wrap mb-3">
                <button
                  ...
                  className={`px-2.5 py-1 text-xs rounded border transition-colors ${
                    channels.length === 0
                      ? 'border-[#1800ad] bg-[#1800ad] text-white font-semibold'
                      : 'border-[#cccccc] text-[#6b6b6b] hover:border-[#1800ad]'
                  }`}
                >
                  All channels
                </button>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold bg-[#1800ad] text-white hover:bg-[#2f1ac9] rounded transition-colors disabled:opacity-40 disabled:pointer-events-none"
                >
        ...
        <div className="w-full lg:w-[360px] shrink-0 space-y-6">
          ...
          <Card title="Describe a rule" sub="Say it in your own words — the AI turns it into a precise, checkable rule.">
            <input
              ...
              className="w-full mb-2 px-3 py-2 text-sm border border-[#cccccc] rounded outline-none focus:border-[#1800ad] disabled:opacity-50"
            />
            <div className="flex items-center gap-3">
              <button
                onClick={draftWithAI}
                disabled={drafting || !description.trim()}
                className="px-4 py-2 text-xs font-semibold bg-[#1800ad] text-white hover:bg-[#2f1ac9] rounded transition-colors disabled:opacity-40 disabled:pointer-events-none whitespace-nowrap"
              >
                    ...
                    <div className="px-3 pb-2.5 flex gap-1.5 flex-wrap bg-white">
                      {SUGGESTIONS.filter(s => s.category === cat).map(s => (
                        <button
                          ...
                          className="px-2.5 py-1 text-[11px] border border-[#cccccc] rounded text-[#6b6b6b] hover:border-[#1800ad] hover:text-[#1800ad] transition-colors"
                        >
                          + {s.label}
                        </button>
                      ))}
